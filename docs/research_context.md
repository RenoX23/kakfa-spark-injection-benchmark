...
| **Zhang, L., Jia, T., Jia, M., Wu, Y., Liu, A., Yang, Y., Wu, Z., Hu, X., Yu, P.S., Li, Y. *A Survey of AIOps for Failure Management in the Era of Large Language Models.* arXiv:2406.11213 (June 2024); ACM Computing Surveys, DOI: 10.1145/3746635. Author list confirmed directly against the arXiv listing, 2026-07-16 — an earlier draft of this row had a placeholder "Chen, Z. et al." attribution that was never actually verified; caught and fixed before being left in, not after.** Added 2026-07-16 saturation re-check — more current than Notaro et al. (2021), not previously cited. | Updated, 2024-current AIOps failure-management survey; extends the field's framing to LLM-based approaches | Broader survey, not streaming-pipeline-specific; does not itself close this project's gap, cited for recency alongside Notaro et al. per the "don't cite only 2018-2021 work" standard |
...
| Porcelli, L. *A Feature Engineering Approach for Business Impact-Oriented Failure Detection in Distributed Instant Payment Systems.* arXiv:2510.21710, submitted 2025-08-07. **Author confirmed 2026-07-16 (single author, Lorenzo Porcelli) — the prior "verify author list" flag is resolved, not just removed.** | Recent (2025) example of AIOps + explainable feature engineering applied to failure detection in a specific distributed-system domain (payments), showing the "domain-specific AIOps benchmark" pattern is an active, current research move | Domain is payments infrastructure, not streaming data pipelines |
| Harrison, C., Kirkpatrick, C.R., Dutra, I. *Bioinformatics Computational Cluster Batch Task Profiling with Machine Learning for Failure Prediction.* arXiv:1812.09537 (2018-12-22). **Correction, 2026-07-16: the author list previously recorded here ("Ahmed, M., Fisher, D. et al.") was wrong — confirmed directly against the paper's own arXiv listing. This was a real citation error, not just an unverified-but-correct one; caught by this saturation re-check, not assumed clean.** | Job-failure prediction on batch HPC clusters via Random-Forest feature selection at production scale (Random Forest, 50 trees; 95.4% overall accuracy; on the failed-job class, 88% recall / 99% precision — the "88%" is recall, not accuracy, confirmed against the PDF 2026-07-19) | Batch HPC scheduling context, not streaming pipelines; no lead-time evaluation |
| **Alharthi, K.A., Jhumka, A., Di, S., Gui, L., Cappello, F., McIntosh-Smith, S. [Time Machine: Generative Real-Time Model for Failure (and Lead Time) Prediction in HPC Systems](https://ieeexplore.ieee.org/document/10202658/). DSN 2023 (IEEE/IFIP Int. Conf. on Dependable Systems and Networks), pp. 508–521. DOI: 10.1109/DSN58367.2023.00054. Added 2026-07-16; venue + full author list corrected 2026-07-19 while building `literature/` (previously recorded only as "IEEE, June 2023" with no authors — the venue was DSN 2023, not a generic IEEE entry). Closest thematic match found (predicts both failure AND lead time, like this project), missing from the first review pass entirely.** | Two-stack transformer-decoder architecture predicting upcoming log-based failure, its location, and lead time, on real historical HPC log data; the only located prior work that names "lead time" as an explicit prediction target the way this project does | HPC log-event domain, not Kafka/Spark telemetry; no controlled fault-injection methodology (learns from naturally-occurring historical failures, not induced ones); no static-threshold-baseline comparison; a novel deep architecture, not a benchmark/methodology contribution — the closest *thematic* neighbor found, still not a competing benchmark |
| Al-Sayeh, H., Hagedorn, S., Sattler, K.-U. *A gray-box modeling methodology for runtime prediction of Apache Spark jobs.* Distributed and Parallel Databases 38, 819–839 (2020). DOI: 10.1007/s10619-020-07286-y. **Full citation resolved 2026-07-19 — prior "verify before citing" flag closed.** | Established Spark job **runtime/performance** prediction as a distinct, well-studied problem from failure prediction | Predicts execution time and resource needs, not failure — useful as a contrast to sharpen your framing, not a competitor to your gap |
| Industry practice (Kafka monitoring vendor content: Acceldata, Conduktor, IBM, AutoMQ, 2025–2026; XGBoost-based Kafka lag-prediction patent, US). **Re-checked 2026-07-16**: a targeted search for a real head-to-head "ML vs. static threshold" Kafka lag-prediction comparison found none — current industry practice as of 2026 still centers on "sophisticated threshold-based approaches with trend analysis," not deployed ML lag prediction, reinforcing rather than closing this project's gap. | Confirms strong practitioner demand for predictive (vs. reactive) Kafka/Spark monitoring; confirms XGBoost/tree-based models are the default practitioner choice for this feature type where ML is used at all | Patents and vendor blog posts are not peer-reviewed benchmarks; none report controlled fault injection, multi-model comparison, or lead-time metrics |
| Vogel, A., Henning, S., Perez-Wohlfeil, E., Ertl, O., Rabiser, R. [A Comprehensive Benchmarking Analysis of Fault Recovery in Stream Processing Frameworks](https://arxiv.org/abs/2404.06203), ACM DEBS 2024 (also arXiv:2404.06203). **Author list added 2026-07-19 (previously none recorded).** | Closest located adjacent *systems-benchmark* work: chaos-engineering-inspired fault injection across Flink, Kafka Streams, and Spark Structured Streaming, comparing **recovery performance, stability, and recovery time** in a cloud-native (Kubernetes) environment | Measures post-hoc recovery speed, not predictive ML; no lead-time-to-failure metric; no static-threshold-vs-ML comparison; no labeled pre-failure dataset; compares three *competing frameworks* against each other rather than treating a fixed Kafka+Spark pipeline as the object of prediction. Confirmed via direct abstract fetch, not assumed from title. |
| **Distributed Stream Processing for Real-Time Healthcare-Motivated Analytics in Multi-Cloud: A Semantics-Aligned Benchmark of Kafka-Centric Pipelines with Flink and Spark Structured Streaming.** *Int. J. Emerging Trends in Computer Science and Information Technology* (March 2026). Added 2026-07-16 — the newest Kafka+Spark-specific benchmark found; a reviewer doing their own current search would likely find this too, addressed proactively. **Venue not independently verified as Scopus/major-indexed — flagged, not vouched for.** | Multi-cloud (AWS/Azure) latency and checkpointing/fault-recovery-time comparison between Flink and Spark Structured Streaming for a healthcare-motivated Kafka pipeline, including quantified recovery-time and p99 latency differences | Same category as DEBS 2024: engine-vs-engine recovery/performance benchmarking, not predictive ML; no fault-injection-based labeled dataset for ML training; no lead-time-to-failure metric; no static-threshold-vs-ML comparison |

**Positioning statement, re-confirmed 2026-07-16 (previous check: 2026-07-08).** Six targeted searches this pass (general Kafka/Spark failure-prediction/fault-injection/lead-time terms, AIOps streaming lead-time terms, Kafka ML-vs-threshold specifically, and direct follow-ups on two new candidates) found two genuinely new sources (Time Machine, the March-2026 healthcare Kafka/Spark benchmark) and one updated survey, none of which close the gap: still no located work combining controlled fault injection + multi-model ML + lead-time evaluation + static-threshold-baseline comparison specifically on a fixed Kafka + Spark Structured Streaming pipeline. The general principle — ML-based failure prediction beats static thresholds, validated via fault injection — is well established in AIOps (Notaro et al., 2021) and has been executed rigorously in adjacent domains (DRAM, batch clusters, and now HPC log-based lead-time prediction specifically via Time Machine). Industry demand for the Kafka/Spark-specific version is clearly present (vendor tooling, a patent) but, per this pass's direct check, not yet met even in current practice — 2026 industry guidance still centers on threshold-plus-trend alerting, not deployed ML lag prediction. This is the gap this project fills. **Re-check again before final submission** — a novelty claim resting on a search is only as current as the search.

**Full verified literature collection (added 2026-07-19).** A curated 20-paper set with PDFs and BibTeX now lives in [`literature/`](../literature/) (start at [`literature/README.md`](../literature/README.md)) — the authoritative, primary-source-verified version of this section. Beyond the rows above it adds the methods/background citations this project actually uses or builds on (Random Forest / XGBoost / LightGBM / SHAP; the Kafka and Spark Structured Streaming system papers; Salfner et al. 2010 online-failure-prediction survey; Basiri et al. 2016 chaos engineering; Natella et al. 2016 software-fault-injection survey) and three further adjacent ML-failure-prediction works (Lin et al. 2018 cloud node-failure prediction; Pham et al. 2025 RCAEval telemetry benchmark, WWW'25 Companion; Chen et al. 2025 Kubernetes failure injection). None changes the gap conclusion above — each is positioned there, not treated as a competitor.
...
4. **A labeled dataset of 47 real, physically-injected fault episodes across 5 classes** (broker_kill 8, disk_pressure 8, network_degradation 8, backpressure_cascade 8, executor_oom 15), each with an onset timestamp recorded by the injection tooling itself at the moment it caused the fault — not inferred, not human-annotated after the fact — plus paired real Prometheus telemetry. This is what makes lead-time computation possible at all, and is the harder property to obtain than raw scale: a same-sized "found" production-incident dataset would have materially weaker ground truth. Documented as a standalone artifact, not just a byproduct of the benchmark: `results/DATASET.md` (schema, per-class field reference, the 20 discarded episodes' documented exclusion reasons, reproduction instructions).
5. A fully open, reproducible benchmark (infra-as-code + fault-injection scripts + the dataset above) — confirmed public, not just intended to be: `github.com/RenoX23/kakfa-spark-injection-benchmark` (verified via GitHub's public API, 2026-07-16 — `"private": false`). The benchmark and dataset are citable artifacts independent of the specific model results, mirroring how SCG/SupplyGraph's value lay partly in being public and reproducible.
6. **A mechanism-verification methodology applied to every positive-looking result before it is trusted** — per-instance ablation and independent SHAP cross-checking, not aggregate metrics alone. This is what caught disk_pressure's 98.6s "lead time" as a static absolute-value discriminator rather than genuine physical precursor detection (Section 6.5's mechanism trace, Section 6.6's SHAP corroboration) — a real, demonstrated catch, not an aspirational claim of rigor. Offered as a template other AIOps benchmark work can reuse to avoid the same class of overclaiming.

**Scope honesty, stated once here rather than left implicit:** contributions 1–3 and 6 concern methodology and measurement rigor; 4–5 concern the dataset and benchmark artifact. None of the six concerns a novel model, architecture, or algorithm — every classifier used (Random Forest, XGBoost, LightGBM) is a stock implementation with standard hyperparameters. The paper's contribution is the measurement apparatus and what it honestly found (Section 6.5's zero-of-five-classes result), not a system that wins.
...
- **Class imbalance** (normal windows vastly outnumber pre-failure windows) — plan for class-weighting or SMOTE from the start, report as an explicit methodological step, not an afterthought. **RESOLVED, Weeks 8-9**: class-weighting applied, not SMOTE — `RandomForestClassifier(..., class_weight="balanced")` (`docs/paper_draft.md` Table 1 header, Section 6.4) — updated 2026-07-19, this entry was stale (still said "open" after the mitigation had already shipped).
...
- **Someone publishes this exact intersection first** — the AIOps space is active (2025 payment-systems paper found in this pass alone). Re-checked 2026-07-08 (Weeks 2-3 saturation re-check, Section 3): gap still open, closest hit (DEBS 2024) answers a different question (recovery speed, not predictive lead time). **Re-checked again 2026-07-16** (writing phase had started — Results section drafted — triggering this entry's own stated re-check condition): gap still open; found one closer thematic neighbor (*Time Machine*, IEEE 2023, HPC log-based failure+lead-time prediction — still not a competing benchmark) and one newer surface-similar Kafka/Spark benchmark (IJETCSIT, March 2026, engine-performance comparison, not predictive ML). Also caught and fixed a real citation error (wrong author list on `arXiv:1812.09537`) while re-verifying, not just re-confirming the gap. Still an active risk — **re-check again before final submission**, not treated as permanently closed by two passes.
...
- **`evaluation_output.json`'s `leads` field meant three different things across four classes, one of them stale — found 2026-07-13 while drafting the paper's Results section, RESOLVED same day.** Section 6.5's ML-vs-baseline table cites baseline lead times of 0.0s (disk_pressure), 57.0s (broker_kill), 64.9s (executor_oom), and a 0s/65s split (network_degradation); attempting to independently re-derive these from `results/baseline-threshold-evidence/evaluation_output.json`'s top-level `leads` field reproduced executor_oom's number exactly but *not* disk_pressure's or network_degradation's. Traced to `evaluate_baselines.py`: `leads` = crossing→OOM (a real crash event) for executor_oom — correct, unchanged; `leads` = crossing→fault-natural-end for disk_pressure/network_degradation — the *pre*-gate-audit-fix definition `docs/baseline_thresholds.md`'s own "Full Evaluation Results" section already describes replacing with `severity_detail.crossing_to_severity_lead_s`, just never removed from the JSON afterward, so it sat beside the correct field under a near-identical name. Confirmed stale by direct recomputation: `severity_detail` reproduces the reported table numbers exactly (disk_pressure 0.0s ×8/8; network_degradation 0s ×3/65s ×1, 4/8 reach severity). Fixed (`results/baseline-threshold-evidence/clarify_leads_fields.py`, also corrected in `evaluate_baselines.py` for future reruns): executor_oom's field renamed to `crossing_to_crash_event_s` (value unchanged), disk_pressure/network_degradation's stale field moved to `leads_crossing_to_natural_end_s_SUPERSEDED` (kept as disclosed history, not deleted, `severity_detail` untouched). No number in any reported table changed.
  **Independent finding while re-verifying broker_kill's "unaffected" case, not assumed clean because the number matched:** broker_kill's `leads` = crossing→`target_recovered_utc`, i.e. residual outage duration *after* detection, not a crossing-to-crash-event lead time — its 57.0s (n=1) figure was never stale or wrong, but it is a different *kind* of quantity than the other three classes' predictive lead times, because a binary up/down signal has no distinct "crash event" later than crossing to lead into. Renamed to `crossing_to_recovery_s` to stop it reading as directly comparable; whether broker_kill belongs in a "baseline lead time" comparison at all is flagged for whoever finalizes Results/Discussion, not decided here. Full trace: `docs/baseline_thresholds.md`'s "Field-naming correction" addendum.
...
- **Zhang, L., Jia, T., Jia, M., Wu, Y., Liu, A., Yang, Y., Wu, Z., Hu, X., Yu, P.S., Li, Y. (2024). *A Survey of AIOps for Failure Management in the Era of Large Language Models.* arXiv:2406.11213; ACM Computing Surveys, DOI: 10.1145/3746635. Confirmed 2026-07-16.**
- **Porcelli, L. (2025). *A Feature Engineering Approach for Business Impact-Oriented Failure Detection in Distributed Instant Payment Systems.* arXiv:2510.21710, submitted 2025-08-07. Author confirmed 2026-07-16 — moved here from "verify before citing," single author, not "et al."**
- **Harrison, C., Kirkpatrick, C.R., Dutra, I. (2018). *Bioinformatics Computational Cluster Batch Task Profiling with Machine Learning for Failure Prediction.* arXiv:1812.09537, submitted 2018-12-22. Author list corrected 2026-07-16 — the previously recorded "Ahmed, M., Fisher, D. et al." was wrong, not just unverified; confirmed against the paper's own arXiv listing.**
- **Alharthi, K.A., Jhumka, A., Di, S., Gui, L., Cappello, F., McIntosh-Smith, S. (2023). [Time Machine: Generative Real-Time Model for Failure (and Lead Time) Prediction in HPC Systems.](https://ieeexplore.ieee.org/document/10202658/) DSN 2023 (IEEE/IFIP Int. Conf. on Dependable Systems and Networks), pp. 508–521. DOI: 10.1109/DSN58367.2023.00054. Confirmed 2026-07-16; venue + full author list added 2026-07-19 (previously recorded only as "IEEE, 2023-06" with no authors — the venue was DSN 2023).**
- **Vogel, A., Henning, S., Perez-Wohlfeil, E., Ertl, O., Rabiser, R. (2024). A Comprehensive Benchmarking Analysis of Fault Recovery in Stream Processing Frameworks. ACM DEBS 2024; arXiv:2404.06203. Author list confirmed 2026-07-19 — previously in the §3 table with no authors and absent from this list.**
- **Al-Sayeh, H., Hagedorn, S., Sattler, K.-U. (2020). A gray-box modeling methodology for runtime prediction of Apache Spark jobs. Distributed and Parallel Databases 38, 819–839. DOI: 10.1007/s10619-020-07286-y. Moved here from "verify before citing" 2026-07-19 — full citation resolved.**
...
- Distributed Stream Processing for Real-Time Healthcare-Motivated Analytics in Multi-Cloud: A Semantics-Aligned Benchmark of Kafka-Centric Pipelines with Flink and Spark Structured Streaming. *Int. J. Emerging Trends in Computer Science and Information Technology* (2026-03). Found 2026-07-16 — full author list not pulled, and the venue's indexing/standing has not been independently verified (not a recognized major venue at time of finding; flagged, not vouched for). Confirm both before citing.
...
**Saturation re-check log (2026-07-16, pre-writing-phase gate per Section 11's own stated trigger).** Six targeted searches: general Kafka/Spark failure-prediction/fault-injection/lead-time terms (2026-dated); AIOps streaming-pipeline lead-time terms; a direct follow-up on the "Time Machine" result found in the first search; both previously-flagged citations' exact author lists; a direct Kafka-lag ML-vs-threshold search; and the updated 2024 AIOps/LLM survey. Findings:
1. **Gap still open** — no paper located combining controlled fault injection + multi-model ML + lead-time evaluation + static-threshold-baseline comparison on a fixed Kafka+Spark Structured Streaming pipeline.
2. **One real citation error found and fixed, not just an unverified-but-correct one**: `arXiv:1812.09537`'s author list was wrong (recorded as "Ahmed, M., Fisher, D. et al.," actually Harrison/Kirkpatrick/Dutra) — this is exactly the class of error the "verify before citing" flag exists to catch, and it caught one.
3. **Two new sources found that were missing from the original review**, added to the table above: *Time Machine* (IEEE 2023) — the closest thematic neighbor located in this whole project (predicts failure AND lead time, like this project does), still not a competitor (different domain, no fault injection, no threshold-baseline comparison, a model contribution not a benchmark one); and a March-2026 Kafka+Spark healthcare benchmark (IJETCSIT) — the newest surface-level-similar paper found, addressed proactively since a reviewer would likely find it too, same DEBS-2024-style category (engine recovery/performance benchmarking, not predictive ML).
4. **One updated survey added** (Zhang et al. 2024, supersedes relying on Notaro et al. 2021 alone for recency).
5. **Industry-practice claim re-checked directly**, not just asserted: current 2026 Kafka monitoring guidance still centers on threshold-plus-trend alerting, not deployed ML lag prediction — reinforces, rather than undermines, the stated gap.

Two items remained open after this pass: the Springer gray-box paper's full citation, and the March-2026 IJETCSIT paper's author list and venue standing.

**Literature-collection build + correction pass (2026-07-19).** Built the standalone verified collection in [`literature/`](../literature/) (20 papers across 5 themes, 17 open-access PDFs + `references.bib`); every entry resolved against its arXiv ID / DOI primary source, author/year/venue read directly off the source page. Corrections folded back into §3 and this section: (1) *Time Machine*'s record was wrong/incomplete — it is DSN 2023 with a six-author list and a DOI, not an anonymous "IEEE 2023" entry; (2) the DEBS 2024 benchmark had **no authors** recorded — added (Vogel, Henning, Perez-Wohlfeil, Ertl, Rabiser); (3) the gray-box Spark paper's "verify before citing" flag is now **closed** (Al-Sayeh, Hagedorn, Sattler, DPD 38, 2020). One item still open: the IJETCSIT March-2026 paper (venue standing unverified — excluded from `literature/`, kept only as an awareness note here). **Re-check the novelty gap again before final submission** — a novelty claim resting on a search is only as current as the search that produced it.

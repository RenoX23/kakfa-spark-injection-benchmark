...
| **[Time Machine: Generative Real-Time Model for Failure (and Lead Time) Prediction in HPC Systems](https://ieeexplore.ieee.org/document/10202658/), IEEE (June 2023). Added 2026-07-16 — a real gap in the original review, this is the closest thematic match found (predicts both failure AND lead time, like this project), and was missing from the first pass entirely.** | Two-stack transformer-decoder architecture predicting upcoming log-based failure, its location, and lead time, on real historical HPC log data; the only located prior work that names "lead time" as an explicit prediction target the way this project does | HPC log-event domain, not Kafka/Spark telemetry; no controlled fault-injection methodology (learns from naturally-occurring historical failures, not induced ones); no static-threshold-baseline comparison; a novel deep architecture, not a benchmark/methodology contribution — the closest *thematic* neighbor found, still not a competing benchmark |
...
| [A Comprehensive Benchmarking Analysis of Fault Recovery in Stream Processing Frameworks](https://arxiv.org/abs/2404.06203), ACM DEBS 2024 (also arXiv:2404.06203) | Closest located adjacent *systems-benchmark* work: chaos-engineering-inspired fault injection across Flink, Kafka Streams, and Spark Structured Streaming, comparing **recovery performance, stability, and recovery time** in a cloud-native (Kubernetes) environment | Measures post-hoc recovery speed, not predictive ML; no lead-time-to-failure metric; no static-threshold-vs-ML comparison; no labeled pre-failure dataset; compares three *competing frameworks* against each other rather than treating a fixed Kafka+Spark pipeline as the object of prediction. Confirmed via direct abstract fetch, not assumed from title. |
...
- **[Time Machine: Generative Real-Time Model for Failure (and Lead Time) Prediction in HPC Systems.](https://ieeexplore.ieee.org/document/10202658/) IEEE (2023-06). Confirmed 2026-07-16 — found this pass, was missing from the original review entirely.**
...
- Gray-box modeling methodology for runtime prediction of Apache Spark jobs. *Distributed and Parallel Databases* (Springer, 2020) — pull full author/citation from Springer page directly. Not touched in the 2026-07-16 pass — still open.
...
Two items remain genuinely open after this pass, not resolved by it: the Springer gray-box paper's full citation, and the March-2026 IJETCSIT paper's author list and venue standing. **Re-check again before final submission** — a novelty claim resting on a search is only as current as the search that produced it.
